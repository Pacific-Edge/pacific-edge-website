...
type ColorBendsProps = {
  ...
  intensity?: number;
  bandWidth?: number;
  /** Attach the pointer-move listener (mouse warp / parallax). Off by default
   *  here — this is a passive background, not an interactive surface. */
  interactive?: boolean;
};
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    if (prefersReduced) {
      // Reduced motion: paint one static frame, no loop, no observer.
      renderFrame();
    } else {
      // Only pay the per-frame shader cost while the section is on (or near)
      // screen — scrolling past it must not keep the WebGL loop spinning.
      let inView = false;
