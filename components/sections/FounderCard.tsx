...
export default function FounderCard({ founder }: { founder: Founder }) {
  ...
  return (
    <article className="flex flex-col">
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-xl bg-midnight-900/5 mb-6">
        {founder.photo ? (
          <Image
            src={founder.photo}
            alt={founder.name}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-midnight-900">
            <span
              className="font-display font-bold text-white-50"
      ...
      <div>
        ...
        <h2 className="font-display text-display-sm text-midnight-900 mb-3">{founder.name}</h2>
        <p className="font-ui text-sm text-midnight-900/60 leading-relaxed">{founder.bio}</p>
      </div>
    </article>
  )
}
