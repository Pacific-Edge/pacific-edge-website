...
| **Tailwind CSS v4**         | Utility-first styling; design tokens (white, midnight, ash) live in `app/globals.css`                     |
...
Stakeholder redesign feedback (Leone + team) is tracked below — **P0 first**.

## Existing launch blockers
...
- [ ] **Visual QA** — spot-check 375 / 768 / 1024 / 1440 breakpoints before go-live (include savings calculators at `/tools/savings/`*)
- [ ] **Lint** — `npm run lint` reports 6 `react-hooks/set-state-in-effect` errors (Next 16 rule) on one-time init effects in `Nav`, `Hero`, `ProcessTeaser`, `AnimatedValue`, `ScriptedChatDemo`, and the portal page; clean these (or scope the rule) before wiring lint into CI
- [ ] **Deploy** — ship to production host; confirm legacy redirects (`public/_redirects` or `vercel.json`)
