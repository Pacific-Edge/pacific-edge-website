  ...
  return (
    <div className="rounded-xl border border-ash-300/40 bg-white-50 p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="font-ui text-sm font-medium text-midnight-900">{data.extraTitle}</p>
        <p className="font-ui text-[10px] uppercase tracking-wide text-midnight-900/40">Handled by the platform</p>
      </div>
        ...
        {data.extra.map(([icon, title, sub, pill, statusClass], i) => (
          <div key={i} className="flex items-center gap-4 py-3.5">
            <div className="w-10 h-10 rounded-lg bg-white-100 border border-ash-300/40 flex items-center justify-center shrink-0">
              <CallIcon name={icon} className="w-4 h-4 text-midnight-700" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-ui text-sm font-medium text-midnight-900">{title}</p>
              <p className="font-ui text-xs text-midnight-900/45">{sub}</p>
            </div>
