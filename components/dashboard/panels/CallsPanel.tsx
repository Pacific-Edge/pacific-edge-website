    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-ash-300/40 bg-cream-50 p-5">
        <div className="flex items-center justify-between mb-4">
          <p className="font-ui text-sm font-medium text-navy-900 flex items-center gap-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border border-navy-900/10 bg-navy-900/[0.03]">
          <div className="w-12 h-12 rounded-xl bg-navy-900/10 flex items-center justify-center shrink-0">
            <CallIcon name="phone" className="w-5 h-5 text-navy-700" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-ui text-sm font-medium text-navy-900">{data.live.name}</p>
            <p className="font-ui text-xs text-navy-900/45 mt-0.5">{data.live.sub}</p>
            <p className="font-ui text-sm text-navy-900/60 italic mt-2">{data.live.line}</p>
          </div>
      ...
      <div className="rounded-xl border border-ash-300/40 bg-cream-50 p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="font-ui text-sm font-medium text-navy-900">Today&apos;s calls</p>
          <p className="font-ui text-[10px] uppercase tracking-wide text-navy-900/40">Tap to expand</p>
        </div>
          ...
          {data.calls.map((call, i) => (
            <div key={`${call.who}-${i}`}>
              <button
                ...
                className="w-full flex items-center gap-4 py-3.5 text-left hover:bg-cream-100/50 rounded-lg px-1 transition-colors"
                aria-expanded={openIndex === i}
              >
                <div className="w-10 h-10 rounded-lg bg-cream-100 border border-ash-300/40 flex items-center justify-center shrink-0">
                  <CallIcon name={call.icon} className="w-4 h-4 text-navy-700" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-ui text-sm font-medium text-navy-900">{call.who}</p>
                  <p className="font-ui text-xs text-navy-900/45">{call.sub}</p>
                </div>
                <StatusPill status={call.status}>{call.pill}</StatusPill>
                <span
                  className={`text-navy-900/30 text-xs transition-transform ${openIndex === i ? "rotate-180" : ""}`}
                  aria-hidden
                >
                  ▼
                </span>
              ...
              {openIndex === i && (
                <p className="pb-4 pl-14 pr-4 font-ui text-sm text-navy-900/65 leading-relaxed">
                  {call.detail}
                </p>
              )}
            </div>
