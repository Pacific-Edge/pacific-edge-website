...
export default function JaniceInsights() {
  useEffect(() => {
    ...
    const openInsights = () => {
      ...
      const chips = top.length ? top.map((x) => '<span style="display:inline-flex;gap:6px;align-items:center;font-size:12px;background:#eafaf3;border:1px solid #b9ecd9;color:#0a7d5d;border-radius:var(--radius);padding:4px 10px;margin:0 6px 6px 0">' + esc(x.k) + ' <b style="color:#04130f">&times;' + x.n + "</b></span>").join("") : ""
      ...
      w.innerHTML = '<div style="background:#fff;max-width:640px;width:100%;max-height:86vh;overflow:auto;border-radius:var(--radius);padding:26px 24px;box-shadow:0 40px 90px -30px rgba(0,0,0,.5)">'
        + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"><h3 style="margin:0;font-family:\'Bebas Neue\',sans-serif;font-size:27px;letter-spacing:.5px;color:#1c1914">Janice Demo &middot; Visitor Messages</h3><button id="peClose" style="border:none;background:#f0ece2;border-radius:var(--radius);width:30px;height:30px;cursor:pointer;font-size:16px;line-height:1">&times;</button></div>'
        ...
        + '<div style="display:flex;gap:10px;margin-top:18px"><button id="peExport" style="flex:1;border:1px solid #b9ecd9;background:#eafaf3;color:#0a7d5d;border-radius:var(--radius);padding:10px;cursor:pointer;font-weight:600;font-size:13px">Export CSV</button><button id="peClear" style="border:1px solid #f0d0c5;background:#fbeee9;color:#c2603f;border-radius:var(--radius);padding:10px 16px;cursor:pointer;font-weight:600;font-size:13px">Clear</button></div>'
        + "</div>"
