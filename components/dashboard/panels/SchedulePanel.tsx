  ...
  return (
    <div className="rounded-xl border border-ash-300/40 bg-white-50 p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="font-ui text-sm font-medium text-midnight-900">{data.schedTitle}</p>
        <p className="font-ui text-[10px] uppercase tracking-wide text-midnight-900/40">This week</p>
      </div>
            ...
            <button
              ...
              className={`relative flex flex-col items-center min-w-[64px] px-3 py-2.5 rounded-xl border transition-colors ${
                dayIndex === i
                  ...
                  : "bg-white-50 text-midnight-900 border-ash-300/40 hover:border-midnight-900/20"
              }`}
            >
              ...
              {count > 0 && (
                <span
                  className={`absolute -top-1 -right-1 min-w-4 h-4 px-1 rounded-full text-[9px] font-medium flex items-center justify-center ${
                    dayIndex === i ? "bg-white-50 text-midnight-900" : "bg-midnight-900 text-white-50"
                  }`}
                >
                  {count}
                </span>
      ...
      {slots.length === 0 ? (
        <p className="font-ui text-sm text-midnight-900/45 py-6 text-center">Nothing scheduled</p>
      ) : (
        <div className="divide-y divide-ash-300/30">
          {slots.map(([time, who, detail, status, statusClass], i) => (
            <div key={i} className="flex items-center gap-4 py-3.5">
              <p className="font-ui text-sm font-semibold text-midnight-700 w-20 shrink-0 tabular-nums">{time}</p>
              <div className="flex-1 min-w-0">
                <p className="font-ui text-sm font-medium text-midnight-900">{who}</p>
                <p className="font-ui text-xs text-midnight-900/45">{detail}</p>
              </div>
